...
source language at the gefärdeten\todo{translate} regions with natural numbers:
...
on all junction points $[c]^n$ in the source program and its domain looks like $max \{i\ |\ i \in tregion(n)\} + 1$.
\end{definition}
...
of $E$ see Barthe et. al. 2006} $\alpha$ denotes if $c$ is \textit{part of} a branching
...
The proof that this definition fulfills the properties from \nameref{sec:soundness} and all other
proofs can be found in the full version of the paper. \todo{cite the full version}
