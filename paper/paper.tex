...
of the program\footnote{most mobile devices use some sort of byte-code e.g. Android} with security
...
according to the results of the sequential execution and the thread id furthermore security level of $i$
...
\footnote{including the ones of spawned threads} to $\textsc{t}_{init}$ and that for every program point $j$, which is an
...
hard to use for real programs. In addition they did not how to implement the type-annotations in a real
language.
