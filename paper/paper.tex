...
\item \textit{Timing channels:} Information can be obtained through the time at which this action
      ...
      between threads. For example the two threaded (modeled with $||$) program \[\text{if}\ h = 1\ \{\text{sleep}(100)\}\ ;\ l := 1\ ||\ \text{sleep}(50)\ ;\ l := 0\]
...
[do] not affect [the] publicly observable behavior of a system'\cite{Zdancewic04}. For most real world
applications this policy is far too strict, because it forbids useful programs like password checkers.\footnote{For a
password checker its necessary to reveal whether the user input was correct or not.} Approaches that allow
