...
At this point a first simplification can be made. Looking a state $s \in ConcState$ we can
...
that we have a sequential execution relation $\leadsto_{seq} \subseteq SeqState \times
SeqState$. The one step execution for the multithreaded language $\leadsto_{seq}
\subseteq (ConcState \times History) \times (ConcState \times History)$ is defined
by the following rules.

\begin{figure}
\begin{align*}
\inferrule{pick(s, h) = ctid \\ s.pc(ctid) = i \\ P[i] \in SeqIns \\
...
takes a set of thread identifiers and returns a new thread identifier at level $l$\footnote{It is
assumed that $fresh_l \neq fresh_{l'} Leftrightarrow l \neq l'$} and $\lambda_{init} : \mathcal{P}
\rightarrow LocState$ produces an initial state with the program pointer at the given program point.
...
Based on this an evaluation relation $\Downarrow_{conc} \subseteq (ConcState \times History) \times
GMemory)$ is defined by
...
\item and typing judgments of the form $se, i \vdash_{seq} s \Rightarrow t$, where
      $s, t \in \text{LType}$, $i \in \mathcal{P}$ and se is a securirty environment.
\end{enumerate}

The intuition of the typing judgment is that if we execute the instruction at program
point $i$ w.r.t. the security environment $se$ and the current type is $s$. Then the
type after the execution is $t$.
...
\begin{align*}
\inferrule{P[i] \in SeqIns \\ se, i \vdash_{seq} s \Rightarrow t}
{se, i \vdash s \Rightarrow t}
\end{align*}
\end{minipage}
\begin{minipage}{.5\textwidth}
\begin{align*}
\inferrule{P[i] = \mathtt{start} pc \\ se(i) \leq se(pc)}
{se, i \vdash s \Rightarrow s}
\end{align*}
\end{minipage}
...
\begin{figure}
\begin{align*}
\inferrule{P[i] = store\ x \\ se(i) \sqcup k \leq \Gamma(x)}
{se, i \vdash_{seq} k :: st \Rightarrow st}
\end{align*}

\begin{align*}
\inferrule{P[i] = ifeq\ j \\ \forall j' \in reg(i), k \leq se(j')}
