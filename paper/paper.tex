...
next of an if respective while instruction is at least after the control dependence region.
...
Zdancewic and Myers approach has to goal to provide a secure concurrent language
...
syntax and type system is too awkward, instead it is a model for studying information flow.

Low-security observational determinism is used to ensure noninterference. The idea is
that to make the naturally nondeterministic system, deterministic from the point of view
of the attacker.
...
Let $T(L) = [M_0(L), M_1(L), \dots]$ be the list of values at location $L$ in the trace $T$. Then
...
The approach also lacks (currently) support for real world languages. For example support of synchronization
